...
## Database Migrations Policy and Guidance

### Local Developer Setup
- Local development utilizes SQL Server / LocalDB or Docker SQL Server container with the connection string configured in User Secrets:
  `dotnet user-secrets set "ConnectionStrings:ClaimDatabase" "Server=(localdb)\\mssqllocaldb;Database=ElixomClaimDb;Trusted_Connection=True;MultipleActiveResultSets=true"`
- Run migrations locally via `dotnet ef database update --project src/ElixomClaim.Lib --startup-project src/ElixomClaim.Web`.

### Production Migration Policy
- Automatic destructive schema operations (such as dropping tables or columns without explicit review) are strictly prohibited.
- Production deployments must utilize a guarded, single-instance migration runner (`ApplyDatabaseMigrationsAsync()`) or a dedicated migration job to prevent concurrent migration races.
- All database objects generated by migrations must reside strictly inside the `dbclaim` schema.

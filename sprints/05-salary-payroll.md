...
| 6 | In progress | 2026-09-03 | Added `/payroll` Accountant workspace, salary-definition creation form, recent payroll status/actions, and scoped audit-history display. Remaining: render calculation previews from `ISalaryPayrollService` rather than the current date candidate. Verified `dotnet build src/ElixomClaim.Web/ElixomClaim.Web.csproj --no-restore` (success). |
| 7 | Not started | — | — |

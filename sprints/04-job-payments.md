...
6. Compose responsive payout HTML with totals, safe bank/payment information, and itemized/subtotalled claims, collection transactions, linked payrolls and entries, deductions, and applicable adjustment context. Never expose internal notes.
...
| 3 | Complete | 2026-09-02 | Added Manager list/detail, compatible Collected-transaction review/attach routes, print-safe detail, and Paid-only resend that clones only existing authorized recipients. Print view test confirms internal notes are excluded. `dotnet test src/ElixomClaim.Web.Tests/ElixomClaim.Web.Tests.csproj --no-restore` passed (26). |
...
| 6 | Complete | 2026-09-02 | Replaced settlement placeholder with responsive, itemized payout HTML containing claims, collections, deductions, totals, payment metadata, and only a masked payee account suffix. Internal notes are excluded. `dotnet test src/ElixomClaim.Lib.Tests/ElixomClaim.Lib.Tests.csproj --no-restore` passed (78). |
| 7 | Complete | 2026-09-02 | Added Accountant Submitted/Scheduled queue and endpoint coverage. `JobPaymentServiceTests` cover lifecycle authority, totals, cross-client rejection, and paid cascade; queue test verifies only actionable statuses. `dotnet build ElixomClaim.slnx --no-restore` and `dotnet test ElixomClaim.slnx --no-restore` passed (78 Lib + 27 Web tests). |
| 8 | Complete | 2026-09-02 | Implemented ADR 0002 linked adjustment workflow: Accountant creates a reasoned partial/full adjustment linked to a Paid original, Administrator approves, and Accountant schedules/settles. Negative amounts are recovery receivables; originals remain immutable. Approval/audit/notification paths reuse the durable payment workflow. Migration: `20260902235348_AddJobPaymentAdjustmentApproval`. Tests: `JobPaymentServiceTests`; `dotnet test src/ElixomClaim.Lib.Tests/ElixomClaim.Lib.Tests.csproj --no-restore` passed (79). |
| 3/6 follow-up | Complete | 2026-09-07 | Restored existing MVC job-payment detail/print projections: collection transactions, deductions, linked payroll entries/workspace link, and adjustment context. No domain lifecycle or schema change. Principal files: `JobPaymentsController`, job-payment/Payroll views, and focused view/controller tests. Verified: `dotnet test src/ElixomClaim.Web.Tests/ElixomClaim.Web.Tests.csproj --no-restore` (70 passed; existing NU1510 warnings only). |

...
| 2 | In progress | 2026-09-09 (/root) | Completed verified checkpoints: production read paths, Razor views, development seed, all OAuth/Claim/Job-Payment lifecycle writes, and Collection record/reissue/queue writes now use `EntityType`/`EntityId`/`OccurredAtUtc` through the typed `AuditEntity` service contract. Remaining callers use the marked legacy adapter. Migration temporarily replaces the trigger only inside its transaction, backfills legacy data, then restores append-only protection; retained legacy columns are nullable for new structured records. Full solution build passed (warnings only); Audit/OAuth tests passed (10), ClaimService tests passed (5), CollectionService tests passed (6), JobPayment/settlement tests passed (10). Remaining: migrate other domain/MCP callers and add Administrator/Manager projection coverage. |
| 3 | Not started | — | — |

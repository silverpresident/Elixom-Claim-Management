...
| 2 | In progress | 2026-09-09 (/root) | Completed a verified checkpoint: production read paths, Razor views, development seed, and focused tests now use `EntityType`/`EntityId`/`OccurredAtUtc`. Migration now temporarily replaces the trigger only inside its transaction, backfills legacy data, then restores append-only protection; retained legacy columns are nullable for new structured records. Structured OAuth attribution is covered in `OAuthServiceTests` (8 passed); Audit/relational trigger tests passed (3), Admin/Profile/JobPayments controller tests passed (9), and MCP tool-boundary tests passed (5). Remaining: replace the string target service contract/callers and add Administrator/Manager projection coverage. |
| 3 | Not started | — | — |

...
| 3 | In progress | 2026-09-09 (/root) | Collection receipts, job-payment summaries/resends, and MCP-approved queues now persist configured `From` and Bcc-only system copies; the system-copy address is no longer a visible recipient. No-payor receipts use the configured sender as authorized `To` with the system copy in `Bcc`. CollectionService tests passed (6); JobPayment/settlement tests passed (10); MCP boundary tests passed (5). Remaining: header-aware retry/admin projections, Bcc redaction evidence, and provider evidence. |

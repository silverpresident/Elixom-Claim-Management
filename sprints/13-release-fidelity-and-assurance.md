...
| 3 | In progress | 2026-09-10 (/root) | Collection receipt/reissue, job-payment summaries/resends, and MCP-approved queues persist configured `From` and Bcc-only system copies; skipped optional-payor records persist the same headers. API evidence now asserts one visible `To` recipient plus Bcc, not a visible system-copy row. `dotnet test ElixomClaim.slnx --no-restore` passed (239 tests), including 7 CollectionService tests and the queue API test. Remaining: retry/projection redaction and provider evidence. |

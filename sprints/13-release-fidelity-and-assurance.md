...
| 2 | In progress | 2026-09-09 (/root) | Completed verified checkpoints: production read paths, Razor views, development seed, all OAuth/Claim/Job-Payment lifecycle writes, and Collection record/reissue/queue writes now use `EntityType`/`EntityId`/`OccurredAtUtc` through the typed `AuditEntity` service contract. Salary/payroll lifecycle events are also typed. Remaining callers use the marked legacy adapter. Migration temporarily replaces the trigger only inside its transaction, backfills legacy data, then restores append-only protection; retained legacy columns are nullable for new structured records. Focused salary/payroll tests passed (3). Remaining: migrate other domain/MCP callers and complete all projection evidence. |
| 3 | Not started | — | — |

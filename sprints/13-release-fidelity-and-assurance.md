...
| 6 | Blocked | 2026-09-10 (/root) | Inventory confirms `ApprovedEmailPreviewService` is the sole concrete production adapter without `ILogger<T>`; it has an uncommitted overlapping edit and cannot be safely changed or exempted without ownership resolution. Existing tests cover revoked refresh tokens, expired bearer-token rejection, and MCP transport cancellation. |
| 7 | Not started | — | — |

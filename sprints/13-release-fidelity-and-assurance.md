...
| 2 | In progress | 2026-09-09 (/root) | Completed a verified checkpoint: production read paths, Razor views, development seed, and focused tests now use `EntityType`/`EntityId`/`OccurredAtUtc`. `AuditEntity` is now the typed audit-service contract; all OAuth lifecycle writes use it, while remaining callers use the marked legacy adapter. Migration temporarily replaces the trigger only inside its transaction, backfills legacy data, then restores append-only protection; retained legacy columns are nullable for new structured records. Full solution build passed (warnings only); Audit/OAuth tests passed (10). Remaining: migrate non-OAuth callers and add Administrator/Manager projection coverage. |
| 3 | Not started | — | — |

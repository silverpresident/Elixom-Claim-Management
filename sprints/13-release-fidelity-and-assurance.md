...
| 7 | Blocked | 2026-09-10 (/root) | Cannot execute release controls locally: requires a designated migration runner/release owner, production-shaped Azure SQL staging, safe email-provider test account, provisioned Google sign-in, independently selected OAuth/MCP reviewer, backup/restore authority, and accountable go/no-go sign-off. The runbooks, local evidence, and [release evidence template](../docs/release-readiness-evidence.md) are ready; no production/release claim is made without these external prerequisites. |
| 8 | Complete | 2026-09-17 (/root) | Split `/collections/create` into active-client selection and `/collections/create/transaction?clientId=…`; invalid/inactive/direct transaction access returns to selection, and the transaction form exposes only the selected client’s active options. Payor details now precede purpose, amount, method, and optional reference. Verified `dotnet test src/ElixomClaim.Web.Tests/ElixomClaim.Web.Tests.csproj --no-restore --filter 'FullyQualifiedName~CollectionCreationWorkflowTests|FullyQualifiedName~WorkflowFieldsCompletionTests'` (6 passed) and `dotnet test ElixomClaim.slnx --no-restore --logger 'console;verbosity=minimal'` (246 passed). Principal files: `CollectionsController`, `Views/Collections/Create.cshtml`, `Views/Collections/Transaction.cshtml`, `CollectionCreationWorkflowTests`. |

...
| 2 | In progress | 2026-09-09 (/root) | Completed a verified checkpoint: production read paths, Razor views, development seed, and focused tests now use `EntityType`/`EntityId`/`OccurredAtUtc`. Migration now temporarily replaces the trigger only inside its transaction, backfills legacy data, then restores append-only protection; retained legacy columns are nullable for new structured records. `AuditServiceTests`/relational trigger test passed (3), and Admin/Profile/JobPayments controller tests passed (9). Remaining: replace the string target service contract/callers and add OAuth/MCP attribution and Administrator/Manager projection coverage. |
| 3 | Not started | — | — |

...
| 2 | In progress | 2026-09-10 (/root) | Production read paths, Razor views, development seed, lifecycle services, collection-client administration, MVC/API, and MCP adapters now use `EntityType`/`EntityId`/`OccurredAtUtc` through `AuditEntity`; the string overload is compatibility-only. Migration temporarily replaces the trigger only inside its transaction, backfills legacy data, then restores append-only protection; retained legacy columns are nullable for new structured records. `dotnet test ElixomClaim.slnx --no-restore` passed (239 tests). Remaining: Manager/API/MCP projection evidence. |
| 3 | In progress | 2026-09-10 (/root) | Collection receipt/reissue, job-payment summaries/resends, and MCP-approved queues persist configured `From` and Bcc-only system copies; skipped optional-payor records preserve the same headers. Retry/send tests prove headers persist to delivery history and Bcc reaches the provider message; Administrator delivery views render only `To`/`From`, never Bcc. All active tests/fixtures now use header fields rather than the obsolete `Recipient` alias. Lib tests passed (130); Web tests excluding the unrelated, currently failing `DevelopmentDataSeederTests` passed (110). Remaining: external safe-account/provider rehearsal evidence. |

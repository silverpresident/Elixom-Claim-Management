...
3. **Clean SQL migration baseline:** `20260903053340_InitialCreate` does not create the `dbclaim` schema and also creates a JobPayments foreign key before CollectionClients. The relational audit integration test therefore cannot apply migrations to a clean SQL Server. This predates the 2026-09-07 profile migration and requires a separately scoped baseline migration repair.

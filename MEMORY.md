...
- **Stage:** Sprint 12 Standard MCP Server and Versioned Operations API in progress; item 5b (clean Guid migration baseline) is active.
...
- **Standard MCP transport (2026-09-07):** The host registers official `ModelContextProtocol.AspNetCore` 2.2.0 stateless Streamable HTTP via `AddMcpServer().WithHttpTransport().WithTools<…>()` and maps the sole MCP endpoint at `/mcp`. It requires the custom Bearer scheme, `mcp:access`, and the MCP rate-limit policy. The six former bespoke `/mcp/*` MVC controllers are removed. Each discovered domain tool resolves its concrete actor using `McpToolActorAccessor` and shared `IActorResolver`. The separately scoped `/api/v1` replacement remains Sprint 12 work. See `Program.cs`, `Mcp/Tools/`, and `sprints/12-standard-mcp-and-api.md` item 3.
- **MCP tool boundary (2026-09-07):** The six domain-scoped tool classes expose 14 stable attributed tool names. Each MCP invocation uses the shared actor resolver and creates a correlated `IsMcp=true` audit event; cancellation is preserved and unexpected adapter failures are redacted. Collection DTOs exclude payor email and internal processing fees. See `McpToolContractTests.cs` and Sprint 12 item 4.

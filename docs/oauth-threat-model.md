...
| **TH-03** | Scope Escalation / Privilege Escalation | Attacker attempts to request unauthorized administrative scopes or execute elevated commands. | Requested scopes are checked against the concrete client's persisted `AllowedScopes` before consent and code issuance. OAuth tokens inherit concrete user identity (`UserId`); transport-scope and domain-role checks are enforced at every endpoint. | **Mitigated** |
| **TH-04** | Client Impersonation | Malicious entity poses as a registered MCP client. | Dynamic registration creates public PKCE S256 clients with no secret; public clients must not submit one. Trusted confidential clients are provisioned separately and must authenticate with a SHA-256 stored-secret hash at token exchange and refresh. | **Mitigated** |
...
Repository tests and the threat model provide implementation evidence, but they do not replace an independent security assessment. Before production release, run the interoperability suite against a conforming MCP client and obtain an external OAuth/security review.
